...
try {
    ...
    /*if ($employeeId) {
        $logStmt = $conn->prepare("INSERT INTO otp_fetch_log (employee_id, company_id, fetch_time) VALUES (?, 2, NOW())");
    ...
    $username = 'aakashcworks@gmail.com';
    $password = 'cqmj sqlz bjzs ekkx';
    ...
    $searchQuery = 'FROM "support@icicilombard.com" SUBJECT "One Time Password" SINCE "' . $today . '"';
    ...
    if ($emails && count($emails) > 0) {
        ...
        if (preg_match('/\b\d{6}\b/', $message, $matches)) {
            ...
            $stmt = $conn->prepare("SELECT id FROM icici_otp_log WHERE otp = ? AND DATE(created_time) = ?");
            ...
            if ($stmt->num_rows === 0) {
                ...
                // Insert new OTP
                $insertStmt = $conn->prepare("INSERT INTO icici_otp_log (otp, created_time) VALUES (?, NOW())");
