...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate fields
    if (empty($template_name) || empty($subject) || empty($body) || $template_id <= 0) {
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE email_templates SET 
                              name = ?,
                              subject = ?,
                              body = ?,
                              signature_type = ?,
        ...
        $stmt->bind_param("sssisi", $template_name, $subject, $body, $signature_type, $custom_fields_json, $template_id);
        ...
        $_SESSION['message'] = 'Error updating template: ' . $e->getMessage();
        $_SESSION['message_type'] = 'danger';
        header("Location: ../edit_template.php?id=$template_id");
        exit;
    }
